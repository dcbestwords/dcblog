...
![](./images/16807677940441.jpg)

![](./images/16807678094584.jpg)
...
当浏览器发现缓存有效时，完全不会请求服务器，直接使用缓存即可得到结果，这种情况，又叫做**强制缓存**
...
它们完整的交互过程如下： ![](./images/16807679160641.jpg)
...
具体的有效期设置，按照下面的流程进行： ![](./images/16807679661158.jpg)
...
服务器无法知道客户端到底有没有像浏览器那样缓存文件，它只管根据请求的情况来决定如何响应 ![](./images/16807683916797.jpg) 很多后端语言搭建的服务器都会自带自己的默认缓存规则，当然也支持不同程度的修改

## 浏览器视角

浏览器在发出请求时会判断要不要使用缓存 ![](./images/16807684240658.jpg)

当收到服务器响应时，会自动根据缓存指令进行处理

![](./images/16807684415449.jpg)
